...
# Diverging pair per the dataviz skill's palette: blue <-> red, neutral gray
# midpoint. Dark endpoints are the blue-450->blue-700 sequential-ramp
# darkening ratio applied to the categorical red, so both arms darken by a
# consistent amount rather than an eyeballed dark red.
_TREND_CMAP = LinearSegmentedColormap.from_list(
    "tws_trend_diverging",
    ["#462124", "#e34948", "#f0efec", "#2a78d6", "#0d366b"],
)
...
def plot_global_trend_map(
    ...
    ax.pcolormesh(
        ...
        overlay,
        transform=ccrs.PlateCarree(),
        cmap=LinearSegmentedColormap.from_list("relief", ["#fcfcfb", "#fcfcfb"]),
    ...
    ax.gridlines(draw_labels=False, linewidth=0.5, color="#e1e0d9", alpha=0.8)

    cbar = fig.colorbar(mesh, ax=ax, orientation="horizontal", pad=0.05, shrink=0.6)
    cbar.set_label(title, color="#0b0b0b")
